...
foreach ($ins as $key => $m) {
	...
	if($m["id_materia"] == $exp[2] && $m["id_alumno"] == $_SESSION["id"]){
		...
		echo '<script>
			...
			window.location = "https://app.emunah.edu.co/inscripto-M/'.$exp[1].'/'.$exp[2].'";
			</script>';
				...
				<form method="post">
					...
					<div class="row">
						...
						<div class="col-md-6 col-xs-12">
							...
							<p>*Solo se podrá inscribir a un Grupo por única vez.</p>';
							...
							foreach ($comisiones as $key => $value) {
								...
								echo '<input type="radio" name="id_comision" value="'.$value["id"].'" required>'.$value["dias"].' - '.$value["horario"].' - Cupos: '.$lugares.' <br>';
